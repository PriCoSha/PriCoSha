...
-- __ following are tables added for our extra features
...
CREATE TABLE GroupTag (
    ...
    veto VARCHAR(20),
    tagtime Timestamp,
    PRIMARY KEY(email_tagger, owner_email, fg_name, item_id),
    FOREIGN KEY(email_tagger) REFERENCES Person(email),
    FOREIGN KEY(owner_email, fg_name) REFERENCES Friendgroup(owner_email, fg_name),
    FOREIGN KEY(item_id) REFERENCES ContentItem(item_id)
);

CREATE TABLE GroupTagPending (
    email_tagged VARCHAR(20),
    email_tagger VARCHAR(20), 
    owner_email VARCHAR(20), 
    fg_name VARCHAR(20), 
    ...
    status VARCHAR(20),
    tagtime Timestamp,
    PRIMARY KEY(email_tagged, email_tagger, owner_email, fg_name, item_id),
    FOREIGN KEY(email_tagged) REFERENCES Person(email),
    FOREIGN KEY(email_tagger, owner_email, fg_name, item_id) REFERENCES GroupTag(email_tagger, owner_email, fg_name, item_id)
);

CREATE TABLE Picture (
    item_id int,
    pic_format VARCHAR(50),
    location VARCHAR(50),
    PRIMARY KEY(item_id),
    FOREIGN KEY(item_id) REFERENCES ContentItem(item_id)
);
